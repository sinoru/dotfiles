...
1. Run `git diff HEAD --stat` to get an overview of changed files and their modification scale
2. If there are no changes, say so and stop
3. Run `git diff HEAD` to get the full diff
4. Launch 3 parallel agents to review the changes independently:
   - **Agent 1 — Convention & Best Practices**: Check changes against project conventions (naming, patterns, access control, language-specific guidelines, etc.)
...
6. Filter out false positives and nitpicks — only report issues a senior engineer would flag
7. **Acknowledge justified complexity** — if an abstraction or pattern earns its place, say so explicitly rather than forcing simplification
8. Present a consolidated review
