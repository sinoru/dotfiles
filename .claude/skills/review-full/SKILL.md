...
2. **Git 사용 가능 여부 확인** — Run `git rev-parse --is-inside-work-tree` to check if git is available. Store the result as a flag for conditional steps below.
...
4. Launch 5 parallel agents to review the target files independently, each from its own perspective:
   ...
   - **Agent 4 — Git History & Context** (git 가용 시에만): Use `git log` and `git blame` on reviewed files to analyze:
   ...
   - **Agent 5 — Security**: 공격자 관점에서 리뷰 대상 코드의 보안 취약점을 검토한다.
     3단계 분석:
     (1) Context Research — Glob/Grep으로 기존 보안 프레임워크, 검증 패턴, 인증 미들웨어 등을 파악
     (2) Comparative Analysis — 리뷰 대상 코드가 기존 보안 패턴을 따르는지, 공격 표면이 존재하는지 비교
   ...
   각 에이전트는 리뷰 대상 파일 전체를 자신의 관점으로 읽고 분석한다.
   파일이 많을 경우 에이전트 내부에서 논리적 그룹핑하여 처리.
